...
function formToTitulo(formData, espesorLabelByValue) {
  ...
  return `Vidrio ${ancho}x${alto} · ${tipoVidrio} · ${proveedor} · ${espLabel} · ${acabado}${
    extras ? ` · ${extras}` : ''
  }`;
}

async function calcularVidrioDB(formData, espesorLabelByValue) {
  const { ancho, alto, cantidad, tipoVidrio, proveedor, espesor, acabado, forma, taladros } = formData;
  ...
  return {
    resultado: {
      ...
      tipoVidrio,
      proveedor,
      espesor: espLabel, // <-- mostramos "10+10"
      espesor_mm: Number(espesor), // <-- guardamos 20
      acabado,
      forma:
        forma === 'rectangular'
          ? 'Rectangular'
                  ...
                  {items.map((it, idx) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-600 mt-1">
                        Subtotal: {it.resultado?.subtotal?.toFixed(2)} € · Total:{' '}
                        {it.resultado?.total?.toFixed(2)} €
                      </div>
                    </div>
                  ))}
                </div>
